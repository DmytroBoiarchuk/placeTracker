...
const PlaceCard = ({ fsq_id }: { fsq_id: string }): JSX.Element => {
  ...
  ).some((savedPlace: string) => savedPlace === fsq_id);
  ...
  function onSaveHandler(save: boolean) {
    ...
    const toStore = save
      ? [...storedArray, fsq_id]
      : storedArray.filter((storedPlaceId: string) => storedPlaceId !== fsq_id);
    localStorage.setItem("savedPlaces", JSON.stringify(toStore));
    setIsSaved(save);
  }
  //fetch place data
  const { data, isPending } = useQuery<PlaceDataInterface>({
  ...
  return (
    <div className={classes.cardExpanded}>
      ...
      >
        ...
        {!isPending && (
          <>
            ...
            <div className={classes.photoContainer}>
              <Gallery data={data!} alt="place-photo" />
            </div>
        ...
        <button onClick={showTipsHandler} className={classes.showTipsButton}>
          <span>Show Tips</span>
        </button>
      </div>
